...
{
	eval = EvalIndiv();
	std::fill(successors.begin(), successors.end(), 0);
...
Individual::Individual(Params & params)
{
	...
	lockedPrefixLength = std::vector<int>(params.nbVehicles, 0);
	for (int r = 0; r < params.nbVehicles; r++)
		lockedPrefixLength[r] = params.dynamicVehicleStates[r].locked_prefix_length;
	chromT = params.activeCustomers;
