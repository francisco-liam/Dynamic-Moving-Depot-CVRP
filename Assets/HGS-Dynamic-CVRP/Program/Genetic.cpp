...
{
	const int nbSequenceCustomers = (int)parent1.chromT.size();
	...
	std::uniform_int_distribution<> distr(0, nbSequenceCustomers - 1);
	...
	int j = start;
	while (j % nbSequenceCustomers != (end + 1) % nbSequenceCustomers)
	{
		result.chromT[j % nbSequenceCustomers] = parent1.chromT[j % nbSequenceCustomers];
		freqClient[result.chromT[j % nbSequenceCustomers]] = true;
		j++;
	}
	...
	{
		int temp = parent2.chromT[(end + i) % nbSequenceCustomers];
		if (freqClient[temp] == false)
		{
			result.chromT[j % nbSequenceCustomers] = temp;
			j++;
		}
	}
