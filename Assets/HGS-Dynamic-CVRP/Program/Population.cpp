...
EvalIndiv Population::reconstructedEvaluation(const Individual & indiv) const
{
	EvalIndiv eval;
	double distance = 0.;
	...
	{
		const std::vector<int>& fullRoute = indiv.chromR[r];
		if (fullRoute.empty()) continue;

		double routeDistance = params.timeCost[0][fullRoute[0]];
		double routeLoad = params.cli[fullRoute[0]].demand;
		double routeService = params.cli[fullRoute[0]].serviceDuration;
		for (int i = 1; i < (int)fullRoute.size(); i++)
		{
			routeDistance += params.timeCost[fullRoute[i - 1]][fullRoute[i]];
			routeLoad += params.cli[fullRoute[i]].demand;
			routeService += params.cli[fullRoute[i]].serviceDuration;
		}
		routeDistance += params.timeCost[fullRoute.back()][0];
	...
	eval.capacityExcess = capacityExcess;
	eval.durationExcess = durationExcess;
	eval.penalizedCost = distance + params.penaltyCapacity * capacityExcess + params.penaltyDuration * durationExcess;
	eval.isFeasible = (capacityExcess < MY_EPSILON && durationExcess < MY_EPSILON);
	return eval;
}
...
void Population::printState(int nbIter, int nbIterNoImprovement)
{
	if (params.verbose)
	{
		...
		{
			if (params.dynamicModeEnabled) std::printf(" | MvFeas %zu %.2f %.2f", feasibleSubpop.size(), getBestFeasible()->eval.penalizedCost, getAverageCost(feasibleSubpop));
			else std::printf(" | Feas %zu %.2f %.2f", feasibleSubpop.size(), getBestFeasible()->eval.penalizedCost, getAverageCost(feasibleSubpop));
		}
		...
		if (params.dynamicModeEnabled)
		{
			if (getBestFeasible() != NULL)
			{
				EvalIndiv fullEval = reconstructedEvaluation(*getBestFeasible());
				std::printf(" | Full%s %.2f", fullEval.isFeasible ? "Feas" : "Inf", fullEval.penalizedCost);
			}
			else std::printf(" | FullNA");
		}
...
{
	if (params.nbActiveCustomers == 0) return 0.;
	int differences = 0;
	for (int j = 1; j <= params.nbClients; j++)
	{
		if (!params.customerActive[j]) continue;
	...
	return (double)differences / (double)params.nbActiveCustomers;
}
