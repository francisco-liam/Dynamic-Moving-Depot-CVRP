...
void LocalSearch::run(Individual & indiv, double penaltyCapacityLS, double penaltyDurationLS)
{
	...
	for (int i = 1; i <= params.nbClients; i++)
		if (params.ran() % params.ap.nbGranular == 0)  // O(n/nbGranular) calls to the inner function on average, to achieve linear-time complexity overall
			std::shuffle(params.correlatedVerticesActive[i].begin(), params.correlatedVerticesActive[i].end(), params.ran);
	...
	for (loopID = 0; !searchCompleted; loopID++)
	{
		...
		for (int posU = 0; posU < (int)orderNodes.size(); posU++)
		{
			nodeU = &clients[orderNodes[posU]];
			int lastTestRINodeU = nodeU->whenLastTestedRI;
			nodeU->whenLastTestedRI = nbMoves;
			for (int posV = 0; posV < (int)params.correlatedVerticesActive[nodeU->cour].size(); posV++)
			{
				nodeV = &clients[params.correlatedVerticesActive[nodeU->cour][posV]];
...
{
	if (isLockedNode(nodeU)) return false;
	if (!canInsertAfter(nodeV)) return false;
...
{
	if (isLockedNode(nodeU) || isLockedNode(nodeX)) return false;
	if (!canInsertAfter(nodeV)) return false;
...
{
	if (isLockedNode(nodeU) || isLockedNode(nodeV)) return false;
...
{
	if (isLockedNode(nodeU) || isLockedNode(nodeV) || isLockedNode(nodeX)) return false;
...
{
	if (isLockedNode(nodeU) || isLockedNode(nodeV) || isLockedNode(nodeX) || isLockedNode(nodeY)) return false;
...
{
	if (nodeU->position <= routeU->lockedPrefixLength || nodeV->position <= routeU->lockedPrefixLength) return false;
...
{
	// In dynamic mode, this operator can reroute predecessor chains across depots,
	// which is hard to constrain without a deeper rewrite. We conservatively skip it.
	if (params.dynamicModeEnabled) return false;

	if (nodeU->position <= routeU->lockedPrefixLength || nodeV->position <= routeV->lockedPrefixLength) return false;
...
bool LocalSearch::swapStar()
{
	...
	{
		if (isLockedNode(nodeU)) continue;
		for (nodeV = routeV->depot->next; !nodeV->isDepot; nodeV = nodeV->next)
		{
			if (isLockedNode(nodeV)) continue;
	...
	for (nodeU = routeU->depot->next; !nodeU->isDepot; nodeU = nodeU->next)
	{
		...
		if (mySwapStar.bestPositionU == NULL) continue;
	...
	for (nodeV = routeV->depot->next; !nodeV->isDepot; nodeV = nodeV->next)
	{
		...
		if (mySwapStar.bestPositionV == NULL) continue;
...
void LocalSearch::loadIndividual(const Individual & indiv)
{
	...
	for (int r = 0; r < params.nbVehicles; r++)
	{
		...
		{
			routes[r].lockedPrefixLength = std::min<int>(indiv.lockedPrefixLength[r], (int)indiv.chromR[r].size());
...
void LocalSearch::exportIndividual(Individual & indiv)
{
	...
	for (int r = 0; r < params.nbVehicles; r++)
	{
		...
		indiv.lockedPrefixLength[r] = routes[r].lockedPrefixLength;
		// Route order is intentionally preserved: route index corresponds to vehicle identity.
		Node * node = depots[r].next;
		while (!node->isDepot)
		{
			if (params.customerActive[node->cour])
			{
				if (pos >= (int)indiv.chromT.size())
					throw std::string("ERROR : movable giant tour overflow during LS export");
	...
	if (pos != (int)indiv.chromT.size())
		throw std::string("ERROR : movable giant tour coverage mismatch during LS export");
...
LocalSearch::LocalSearch(Params & params) : params (params)
{
	...
	for (int i : params.activeCustomers) orderNodes.push_back(i);
